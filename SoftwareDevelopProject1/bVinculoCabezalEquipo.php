...
<html>	
	<head>
		<title>Buscar Vinculo Cabezal/Equipo</title>
		<!-- Elaborado: Alejandro Juarez
		Fecha de creacion:30/03/2012 11:15a.m
		...
		<script language="javascript">
			...
			//Entrada: id 
			//Salida: Ninguna
			//Funcionamiento:Obtiene los id del documento.
			function get(id){
			...
			*/
			function validar(){
				if(!get("rbtCabezal").checked && !get("rbtEquipo").checked){
								alert("Seleccione el tipo de placa.");  //cambiar msj
								return false;
	...
	<body>
		...
		<div id="div">
			<h1> Buscar Vínculo cabezal/equipo</h1>
			<p>Seleccione un cabezal o equipo, para buscar su v&iacute;nculo.</p>
			<form method="post" action="bVinculoCabezalEquipo.php" name="frmBuscarVinculo" id="frmBuscarVinculo">
				...
				<table id="tabla">
					...
					<tr>
						...
						<td colspan="2">
							<input type="radio"  id="rbtCabezal" name="rbtTipo" onClick="habilitar(sltCabezal);deshabilitar(sltEquipo)" /> Placa de cabezal:
						</td>
					
						<td>
							<select id="sltCabezal" name="sltCabezal" disabled="true">
								<option value="selec">--Seleccionar--</option>	 <!-- estas opciones son las decabezal -->
								<?php
					...
					<br>
						...
						<td>
							<select id="sltEquipo" name="sltEquipo" disabled="true">
								<option value="selec">--Seleccionar--</option>		<!-- estas opciones son las de equipo-->
								<?php
		...
		<?php
			...
			} else if(array_key_exists('sltEquipo',$_POST)){
